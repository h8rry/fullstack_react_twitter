...
class Post extends Component {
  ...
  newPost = (e) => {
    ...
    })).then(handleErrors).catch(error => {
      this.setState({error: 'Could not add tweet'})
  ...
  deletePost(id, user) {
    ...
    if (user == this.state.logged_user) {
      ...
      fetch(`/api/tweets/${id}`, safeCredentials({method: 'DELETE'})).then(handleErrors).catch(error => {
  ...
  componentDidMount() {
    fetch('/api/authenticated').then(handleErrors).then(data => {
      console.log(data)
      this.setState({logged_user: data.username})
    })
        ...
        <div className="border_write_tweet">
          <form onSubmit={
            ...
            <div class="form-group">
              <textarea class="form-control" rows="3" placeholder="What is happening?"
                onChange={
                  this.handleChange
                }
                value={
          ...
          <div className="border_tweets">
            <div className="container">
              <div className="row">
                ...
                <div className="col-4">
                  <button onClick={
                      () => this.deletePost(tweet.id, tweet.username)
                    }
                    type="button"
